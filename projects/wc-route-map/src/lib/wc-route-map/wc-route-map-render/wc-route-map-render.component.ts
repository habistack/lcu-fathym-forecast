import { Component, Injector, ViewChild, ViewChildren, ChangeDetectorRef, AfterViewInit, QueryList } from '@angular/core';
...
@Component({
  selector: 'forge-wc-route-map-render',
  templateUrl: './wc-route-map-render.component.html',
  styleUrls: ['./wc-route-map-render.component.scss']
})

export class ForgeWcRouteMapRenderComponent extends ForgeGenericControl<ForgeWcRouteMapDetails, ForgeWcRouteMapConfig>
  implements IControlRender<ForgeWcRouteMapDetails, ForgeWcRouteMapConfig> {
  //  Fields
  ...
  constructor(
    ...
    protected wcConfig: WeatherCloudConfigContext,
 ...
 this.routeSubscription = this.notificationService.RouteChanged.subscribe(data => {
  ...
  //   this.wcSvc.ShapeRegion(activeShapeArray).subscribe(data => {
  ...
  protected handleRoute() {
    ...
    this.wcSvc.HandleRoute(this.SearchModel)
      .pipe(
        map(res => res),
        catchError(err => {
      ...
      .subscribe(
        (res) => {
          ...
          this.wcRouteSearch.Search(res as WeatherCloudRouteDataModel);
  ...
  //  const sub: Subscription = this.wcSvc.LoadDataSources().subscribe(dataSources => {
   ...
   */
  protected geofenceDrawingSetup(data: WeatherCloudSearchModel): void {
   ...
   */
  protected geofenceDrawingCallback(callbackData: Array<Array<number>>, searchData: WeatherCloudSearchModel): void {
    this.IsLoading = true;

    this.wcSvc.ShapeRegion(callbackData, searchData)
    .pipe(
      map(res => res),
      catchError(err => {
