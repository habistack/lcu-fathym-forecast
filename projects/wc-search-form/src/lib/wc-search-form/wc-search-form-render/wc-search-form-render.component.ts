...
import {
  ...
  NotificationService,
  ToastrDisplayService,
  WeatherCloudRouteDataContext,
  WeatherCloudRouteDataModel,
  WeatherCloudSearchConstants,
  WCDatasourceTypesModel,
...
// import { ConfigModel } from '@lowcodeunit/lcu-select-common';

// force change

@Component({
  selector: 'forge-wc-search-form-render',
  templateUrl: './wc-search-form-render.component.html',
  styleUrls: ['./wc-search-form-render.component.scss']
})
export class ForgeWcSearchFormRenderComponent extends ForgeGenericControl<ForgeWcSearchFormDetails, ForgeWcSearchFormConfig>
  implements OnDestroy, IControlRender<ForgeWcSearchFormDetails, ForgeWcSearchFormConfig> {
  //  Fields
   ...
   */
  public ButtonLabelDepartTable: string = WeatherCloudSearchConstants.SHOW_DEPARTURE_TABLE;

  /**
   * Label for geofence button
   */
  public ButtonLabelGeofence: string = WeatherCloudSearchConstants.START_POLYGON_DRAWING;

  /**
   * Lable for routes button
   */
  public ButtonLabelRoutes: string = WeatherCloudSearchConstants.SHOW_ROUTES;

  /**
   * Primary Material raised button type
   */
  public ButtonTypePrimary: string = WeatherCloudSearchConstants.BUTTON_PRIMARY;

  /**
   * Warn Material raised button type
   */
  public ButtonTypeWarn: string = WeatherCloudSearchConstants.BUTTON_WARN;

  /**
   * Config for conditions select options
   */
  public ConditionsConfig: any = WeatherCloudSearchConstants.CONDITION_CONFIG;

  public DepartureTimeConfig: any = WeatherCloudSearchConstants.DEPARTURE_TIMES_CONFIG;

  public ForecastModelConfig: any = WeatherCloudSearchConstants.FORECAST_MODEL_CONFIG;

  /**
   * Selected value from data source select
   */
  protected dataSourceSelected: WCDatasourceTypesModel;
  ...
  // public ConditionVariablesNameList: Array<object> = WeatherCloudSearchConstants.VAR_NAMES_SELECT;
...
/**
  ...
  constructor(
    ...
    protected wcSearch: WeatherCloudSearchContext,
    protected wcSearchDT: WeatherCloudSearchDepartureTableContext,
    protected wcRouteSearch: WeatherCloudRouteDataContext,
    protected wcSvc: WeatherCloudService,
    protected wcConfigCtx: WeatherCloudConfigContext,
  ...
  public LoadDataSourceTypes() {
    ...
    const toSelect = this.ForecastModelList.find((c: any) => c.Name.toUpperCase() ===
                                                  WeatherCloudSearchConstants.FORECAST_MODEL_BLEND.toUpperCase());
    ...
    */
   public SelectedDataSourceType(select: WCDatasourceTypesModel): void {

    if (!select) { return; }

     this.dataSourceSelected = new WCDatasourceTypesModel(select.FcstCfg, select.Host, select.Name, select.URLPrefix, select.VarNames);
   }
    ...
    */
  protected getFormValues(searchType: string): WeatherCloudSearchModel {
    let origin: string = '';
    ...
    return new WeatherCloudSearchModel(origin, destination, departTime, showAlternatives, searchType, this.dataSourceSelected);
  }
